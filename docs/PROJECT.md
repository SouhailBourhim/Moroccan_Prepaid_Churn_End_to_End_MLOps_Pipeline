...
│       ├── train_features.parquet # 2,154,048 × 45 (44 features + CHURN)
│       ├── test_features.parquet  # 380,127 × 44
...
│
├── tests/
│   ├── test_features.py           # 17 tests — every transformer + pipeline
│   ├── test_models.py             # 15 tests — evaluate utilities, build_candidates, tune
...
| … | 32 of 44 total features | \|ρ\| > 0.10 | — |

32 of 44 features have |ρ| > 0.10, confirming the feature engineering adds genuine signal beyond the raw columns.
...
```python
{"model": CatBoostClassifier(...), "feature_cols": [...44 names...]}
```
...
### 12.3 Configuration via Environment Variables
...
**Coverage:** 46 tests across 3 test files.
...
46 tests across 3 files. No real model files or data required — all tests run in CI from a clean install.

### `tests/test_features.py` (17 tests)
...
|------|----------|-------------|
| Data drift monitoring | High | Detect when live subscriber distributions diverge from training data. Evidently or WhyLogs can compare feature distributions between the training parquet and a batch of scored subscribers. |
...
| React monitoring dashboard | Low | The `dashboard/` directory is scaffolded. A lightweight Next.js app showing live prediction volume, churn rate trends, and feature drift plots would complete the production monitoring story. |
